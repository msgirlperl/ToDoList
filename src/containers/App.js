...
class App extends Component {
  ...
  */
  onTaskClicked = taskId => {
    ...
    let taskGroup = this.state.taskGroups.find(
      gp => gp.groupName === this.state.selectedTaskGroupName
    );
  ...
  isLocked(task, findTask) {
    ...
    let someParentLocked = parentIds.some(pId => {
      ...
      return (
        parentTask.completedAt === null || this.isLocked(parentTask, findTask)
      );
    });
  ...
  render() {
    const selectedTaskGroup = this.state.taskGroups.filter(
      taskGroup => taskGroup.groupName === this.state.selectedTaskGroupName
    )[0];
      ...
      <div className="container">
        <header className="App-header row">
          <div className="col">
            ...
            <span className="App-logo App-header-text text-uppercase">
              Semi-interesting To-Do List
            </span>
          </div>
        </header>
        <div className="row">
          <div className="col-sm-5">
            ...
            <TaskGroupList
              ...
              groupClick={this.onGroupClicked}
            />
          </div>
          ...
          <div className="col-sm-6">
            <h4>Task Group</h4>
            <TaskList
              tasks={selectedTaskGroup.groupTasks}
              taskClick={this.onTaskClicked}
            />
          </div>
        </div>
          ...
          <div className="collapse" id="collapseExample">
            <div className="text-left panel panel-info">
              <div className="panel-heading">
                The instructions below were provided for this coding exercise
              </div>
              <div className="panel-body">
                <em>Create a grouped task list with task dependencies: </em>
                <ul className="list-group mb-5">
                  <li className="list-group-item">
                    1. Build a task list UI in React using included design and
                    SVG assets
                  </li>
                  <li className="list-group-item">
                    2. The top level should show a list of task groups w/ # of
                    tasks inside
                  </li>
                  <li className="list-group-item">
                    3. Clicking a group should display the list of all tasks for
                    that group
                  </li>
                  <li className="list-group-item">
                    4. Tasks remain locked until all dependent tasks are
                    complete
                  </li>
                  <li className="list-group-item">
                    5. Clicking a task should mark it as complete or incomplete,
                    unless the task is locked
                  </li>
                  <li className="list-group-item">
                    6. Dependencies that do not point to a loaded task should be
                    ignored
                  </li>
                </ul>
